...
fn validate_non_empty_command(
    ...
    if command[0].is_empty() {
        return Err(ProtocolError::InvalidMessageOwned(format!(
            "{field} first segment must not be empty"
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn validate_buffer_pipe_record_rejects_stopped_without_stop_reason() {
        let record = BufferPipeRecord {
            command: vec!["tee".to_owned()],
            state: BufferPipeState::Stopped,
            pid: None,
            exit_code: Some(0),
            stop_reason: None,
        };

        let error = validate_buffer_pipe_record(&record, "buffer_record.pipe")
            .expect_err("stopped pipe requires stop reason");

        assert!(matches!(
            error,
            ProtocolError::InvalidMessageOwned(message)
                if message == "buffer_record.pipe.stop_reason required for stopped pipe"
        ));
    }

    #[test]
    fn buffer_record_validate_rejects_running_pipe_with_exit_code() {
        let record = BufferRecord {
            id: BufferId(1),
        ...
        let error = record
            .validate()
            .expect_err("running pipe cannot set exit code");

        assert_eq!(
            error,
            "buffer_record.pipe.exit_code requires a stopped pipe"
        );
    }

    #[test]
    fn buffer_record_validate_rejects_empty_pipe_program() {
        let record = BufferRecord {
            id: BufferId(1),
            title: "shell".to_owned(),
            command: vec!["sh".to_owned()],
        ...
        let error = record
            .validate()
            .expect_err("pipe command program cannot be empty");

        assert_eq!(
            error,
            "buffer_record.pipe.command first segment must not be empty"
        );
    }
    ...
    #[test]
    fn encode_buffer_request_rejects_empty_start_pipe_program() {
        let error = encode_client_message(&ClientMessage::Buffer(BufferRequest::StartPipe {
            request_id: RequestId(1),
            buffer_id: BufferId(7),
            command: vec![String::new()],
        ...
        .expect_err("empty start pipe program should be rejected");
    ...
    #[test]
    fn decode_buffer_request_rejects_empty_start_pipe_program() {
        let mut builder = FlatBufferBuilder::new();
        let empty_program = builder.create_string("");
        let command = builder.create_vector(&[empty_program]);
        let request = fb::BufferRequest::create(
            &mut builder,
            &fb::BufferRequestArgs {
                op: fb::BufferOp::StartPipe,
