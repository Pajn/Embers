use mux_client::{ClientState, MuxClient, ScriptedTransport};
use mux_core::{
...
fn session_snapshot(root_active: usize, nested_active: usize) -> SessionSnapshot {
    ...
    SessionSnapshot {
        session: SessionRecord {
            ...
            floating_ids: vec![mux_core::FloatingId(90)],
...
fn buffer_detach_focus_and_invalidation_events_update_cache() {
    ...
    state.apply_event(&ServerEvent::FocusChanged(FocusChangedEvent {
        ...
        focused_floating_id: Some(mux_core::FloatingId(90)),
    }));
...
fn node_and_floating_events_mark_session_dirty() {
    ...
    state.apply_event(&ServerEvent::FloatingChanged(FloatingChangedEvent {
        session_id: SessionId(1),
        floating_id: Some(mux_core::FloatingId(90)),
    }));
...
async fn reconnect_resync_rebuilds_sessions_and_detached_buffers() {
    ...
    transport.push_exchange(
        ...
        ServerResponse::Sessions(mux_protocol::SessionsResponse {
            request_id: RequestId(1),
    ...
    transport.push_exchange(
        ClientMessage::Buffer(mux_protocol::BufferRequest::List {
