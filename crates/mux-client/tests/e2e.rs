...
use mux_client::{MuxClient, PresentationModel, Renderer};
use mux_core::{
...
use mux_test_support::{TestConnection, TestServer, cargo_bin};

fn run_cli(server: &TestServer, args: &[&str]) -> Output {
    let output = cargo_bin("mux-cli")
        .arg("--socket")
...
async fn create_buffer(connection: &mut TestConnection, title: &str) -> mux_protocol::BufferRecord {
    let response = connection
...
fn node(snapshot: &SessionSnapshot, node_id: NodeId) -> &mux_protocol::NodeRecord {
    snapshot
        .nodes
        .iter()
...
fn session_id_by_name(client: &MuxClient<mux_client::SocketTransport>, name: &str) -> SessionId {
    client
        .state()
        .sessions
        .values()
...
async fn refresh_all_snapshots(client: &mut MuxClient<mux_client::SocketTransport>) {
...
async fn render_session(
    client: &mut MuxClient<mux_client::SocketTransport>,
    session_name: &str,
) -> String {
