use mux_core::{SplitDirection, new_request_id};
use mux_protocol::{
...
fn tabs_record(snapshot: &SessionSnapshot, node_id: mux_core::NodeId) -> TabsRecord {
    snapshot
        .nodes
        .iter()
...
fn split_record(snapshot: &SessionSnapshot, node_id: mux_core::NodeId) -> SplitRecord {
    snapshot
        .nodes
        .iter()
        .find(|node| node.id == node_id)
